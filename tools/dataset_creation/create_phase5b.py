...
Given:
 i. Phase 5a Crowded Person annotations (+ crowd labels, etc.)
 ii. Multi-label annotations
create image directories for attribute segmentation annotation.

In the previous process, we skip images which contains crowd labels (since it's expensive to annotate).
Now, the idea is to annotate attributes in these images.
"""
import json
import time
import pickle
...
def create_phase5b_fold(fold_name):
    ...
    batch_anno_filenames = filter(lambda x: osp.splitext(x)[0].isdigit(), batch_anno_filenames)
